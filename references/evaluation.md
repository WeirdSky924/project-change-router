...
The runtime reports `enforcement_mode=review_only` if enforcement is missing or disabled, metrics are missing or outside `[0,1]`, any threshold is below its floor, curated provenance is incomplete, the attestation is absent, its evaluation-engine version is missing or stale, or its digest no longer matches route-affecting bundle truth. A plausible top-1 capability match does not override that stop. Read-only analysis, profile repair, and evaluation maintenance may continue; unattended product writes may not.
